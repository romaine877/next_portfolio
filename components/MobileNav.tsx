...
type MobileNavProps = { toggle: () => void };

export default function MobileNav({toggle}: MobileNavProps) {
  ...
  return (
    ...
    className="bg-stone-500 h-full backdrop-blur-sm w-full z-40 fixed inset-0 bg-opacity-25 items-center justify-center">
      <div className="fixed top-5 right-5 max-w-sm dark:bg-[#0b1120] bg-indigo-50 rounded-lg p-5 m-5"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex flex-col justify-between">
          <nav className=" md:space-x-7 md:space-y-0 space-y-1 md:py-3 flex flex-col md:flex-row text-center">
            <Link href="/">
              <a className="text-black dark:text-white p-3 hover:text-indigo-500 hover:bg-white dark:hover:text-indigo-500 hover:shadow-lg dark:bg-transparent dark:hover:bg-violet-50 hover:scale-105 rounded-lg transition ease-in">
                Home
              </a>
            </Link>
            <Link href="/projects">
